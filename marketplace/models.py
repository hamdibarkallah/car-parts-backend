...
class Order(models.Model):
    STATUS_CHOICES = (
        ('PENDING', 'Pending'),           # Client created order, waiting for supplier validation
        ('SUPPLIER_PENDING', 'Supplier Pending'),  # Order created, waiting for supplier approval
        ('APPROVED', 'Approved'),         # Supplier approved order
        ('REJECTED', 'Rejected'),          # Supplier rejected order
        ('PAID', 'Paid'),                 # Client paid for approved order
    ...
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='SUPPLIER_PENDING')
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    supplier_notes = models.TextField(blank=True, null=True, help_text="Notes from supplier when approving/rejecting order")
